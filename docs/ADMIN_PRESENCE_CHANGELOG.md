...
- Built the application once at `458b4e96281041b40ef4197f1ae48c4f052386af` and reused its frozen 91-file payload for `rustdeskadmin-client-2.2.1-x64.msi`, `rustdeskadmin-client-2.2.1-install.exe`, and `rustdeskadmin-client-2.2.1-portable.zip`. Tag commit `f326ef46b1bcf19941cafc62723ad8b901a3b41a` adds documentation only, with no runtime/build/package-source changes. All Windows artifacts remain unsigned; no `RustDeskDeploy.exe` wrapper is included. SFX `CompanyName` remains intentionally blank; the MSI upstream contact is unchanged.
- Published **v2.2.1 as Latest** on 2026-09-10 UTC, neither draft nor prerelease, with exactly three packages. See the [release notes](https://github.com/hrezenmsft/rustdeskadmin-client/releases/tag/v2.2.1) for downloads and SHA-256 checksums. Draft-stage and public HTTPS downloads matched the originals and GitHub digests.
- Retired all three old v2.2.0 binary assets after verification and backup; its release page, unchanged tag, and automatic source archives remain, with replacement links in the description. The paired server's 17-asset retirement also completed (20 old assets total).
- MSI and ZIP extracted hashes match all 91 frozen payload files; SFX contains the complete 24,093,932-byte blob. Validation covered runtime version and package/static/signature/hash checks, not installation smoke tests. Temporarily paused workflows were restored without unwanted rebuilds.
