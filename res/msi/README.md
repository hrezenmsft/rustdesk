...
Release **v2.2.1**, published as **Latest** on **2026-09-10 UTC** (neither draft nor prerelease), includes `rustdeskadmin-client-2.2.1-x64.msi` (MSI version **2.2.1.0**, using `-v 2.2.1 --revision-version 0`) alongside `rustdeskadmin-client-2.2.1-install.exe` and `rustdeskadmin-client-2.2.1-portable.zip`. See the [release notes](https://github.com/hrezenmsft/rustdeskadmin-client/releases/tag/v2.2.1) for downloads and SHA-256 checksums. All three packages reuse the frozen 91-file Windows x64 payload built from `458b4e96281041b40ef4197f1ae48c4f052386af`; tag commit `f326ef46b1bcf19941cafc62723ad8b901a3b41a` adds documentation only, with runtime/build/package source verified unchanged. MSI and ZIP hashes match all 91 payload files. Product name: **RustDeskAdmin - RustDesk Fork**, retaining upstream copyright and adding Henrique Rezende's attribution. The MSI upstream contact remains unchanged; SFX `CompanyName` is intentionally blank.

All three draft-stage and public HTTPS downloads matched the originals and GitHub SHA-256 digests. The three old v2.2.0 assets were then retired, retaining its page, unchanged tag, and automatic source archives with replacement links. Validation covered runtime version and package/static/signature/hash checks, **not a real MSI installation or installation smoke test**.
